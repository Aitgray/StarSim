...
if TYPE_CHECKING:
    ...
    from .model import (
        EventDef,
    )  # Keep this for type hinting in EventDef in evaluate_event_conditions


def evaluate_event_conditions(
    event_def: EventDef, world: World, state: UniverseState
) -> float:
    """
    ...
    for condition in event_def.conditions:
        ...
        if condition_type == "world_unrest":
            if world.unrest >= condition.get(
                "min", 0.0
            ) and world.unrest <= condition.get("max", 1.0):
            ...
            if world.scarcity >= condition.get(
                "min", 0.0
            ) and world.scarcity <= condition.get("max", 1.0):
                multiplier *= weight_multiplier
                condition_met = True
            ...
            if world.tech >= condition.get("min", 0.0) and world.tech <= condition.get(
                "max", 1.0
            ):
                multiplier *= weight_multiplier
                condition_met = True
            ...
            for lane in state.lanes_from(world.id):
                if lane.hazard >= condition.get(
                    "min", 0.0
                ) and lane.hazard <= condition.get("max", 1.0):
                    multiplier *= weight_multiplier
                    condition_met = True
...
def generate_events(
    state: UniverseState, max_events_per_tick: int = 1
) -> List[tuple[EventDef, WorldId]]:
    """
    ...
    weights = [
        item[2] for item in possible_events_with_weights
    ...
    if weights and sum(weights) > 0:  # Ensure there are positive weights to choose from
        chosen_events = state.rng.choices(
            events_only, weights=weights, k=min(max_events_per_tick, len(events_only))
        )
        selected_events = [
            (event_def, world_id) for event_def, world_id, _ in chosen_events
        ]
