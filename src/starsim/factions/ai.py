...
def compute_world_value(faction: Faction, world: World, state: UniverseState) -> float:
    ...
    if world.planets and faction.resource_desire > 0:
        ...
        score += (
            total_planet_potential * faction.resource_desire * 0.5
        )  # Scale by resource_desire and a constant
    ...
    if "expansionist" in faction.traits:
        ...
        if world.planets:
            for planet in world.planets:
                ...
                score += (
                    planet.resource_potentials.get(CommodityId("minerals"), 0.0) * 1.5
                )
    ...
    if "aggressive" in faction.traits:
        ...
        if world.planets:
            for planet in world.planets:
                ...
                score += (
                    planet.resource_potentials.get(CommodityId("energy"), 0.0) * 1.0
                )
    ...
    if (
        ...
        and faction.capital_world_id in state.worlds
        and world.id != faction.capital_world_id
    ):
        capital_world = state.worlds[faction.capital_world_id]
        distance = math.sqrt(
            (world.x - capital_world.x) ** 2 + (world.y - capital_world.y) ** 2
        )
    ...
    if world.factions:
        if world.factions.control == faction.id:
            ...
            if faction.id in world.factions.influence:
                score += (
                    world.factions.influence[faction.id] * 5.0
                )  # Our influence makes it slightly more valuable
            for other_fac_id, influence in world.factions.influence.items():
                if (
                    other_fac_id != faction.id
                    and other_fac_id == world.factions.control
                ):  # Opposing control
                    score -= (
                        influence * 10.0
                    )  # Significantly reduce value if strong opposing control
...
def select_action(faction: Faction, state: UniverseState) -> Dict[str, Any]:
    ...
    controlled_worlds = [
        w_id
        for w_id, w in state.worlds.items()
        if w.factions and w.factions.control == faction.id
    ]
    ...
    for controlled_w_id in controlled_worlds:
        for neighbor_w_id in state.neighbors(controlled_w_id):
            ...
            if (
                not neighbor_world.factions
                or neighbor_world.factions.control != faction.id
            ):
    ...
    if faction.traits and "expansionist" in faction.traits:  # Example heuristic
        for world_id, world in state.worlds.items():
            if world.factions and world.factions.control == faction.id:  # Owned world
                ...
                if (
                    world.market
                    and world.market.inventory.get(CommodityId("minerals")) >= 10.0
                ):
    ...
    if faction.traits and "aggressive" in faction.traits:  # Example heuristic
        for world_id, world in state.worlds.items():
            if world.factions and world.factions.control == faction.id:  # Owned world
                ...
                if (
                    world.market
                    and world.market.inventory.get(CommodityId("alloy")) >= 5.0
                ):
