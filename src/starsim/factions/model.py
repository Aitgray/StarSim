...
class Faction:
    ...
    capital_world_id: Optional[WorldId] = (
        None  # New: The ID of this faction's capital system
    )
...
class WorldFactionState:
    ...
    influence: Dict[FactionId, float] = field(
    ...
    def resolve_control(self):
        ...
        if self.control is None:
            ...
            if (
                highest_influence >= self.control_threshold_gain
                and self.influence.get(self.control, 0.0) < self.control_threshold_loss
            ):
                self.control = current_dominant_faction
