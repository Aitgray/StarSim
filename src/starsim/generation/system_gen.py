...
def _get_connected_components(
    worlds: Dict[WorldId, World], lanes: Dict[LaneId, Lane]
) -> List[Set[WorldId]]:
    """
...
def _get_closest_worlds_between_components(
    component1: Set[WorldId], component2: Set[WorldId], worlds: Dict[WorldId, World]
    ...
    for world_id_a in component1:
        for world_id_b in component2:
            ...
            distance = math.sqrt(
                (world_a.x - world_b.x) ** 2 + (world_a.y - world_b.y) ** 2
            )
...
def generate_planet(
    ...
    if planet_names_data is None:
        planet_names_data = {}
    # Sample habitability
    h_bands, h_weights = _process_weights_for_random_selection(
        planet_type_data["habitability_distribution"]
    )
    ...
    for resource_id_str, potential_data in selected_resource_table[
        "potentials"
    ].items():
    ...
    name = rng.choice(
        planet_names_data.get(planet_type_id, [f"{planet_type_id.capitalize()} Planet"])
    )
...
def generate_world(
    ...
    planets: List[Planet] = []
    planet_type_choices, planet_type_weights = _process_weights_for_random_selection(
        list(planet_types_data.values())
    )

    for _ in range(num_planets):
        selected_planet_type_data = rng.choices(
            planet_type_choices, weights=planet_type_weights, k=1
        )[0]
        planet = generate_planet(
            rng, selected_planet_type_data, planet_names_data
        )  # Pass planet_names_data
    ...
    world_x = max(
        WORLD_COORD_RANGE_X[0],
        min(
            WORLD_COORD_RANGE_X[1], rng.gauss(WORLD_COORD_CENTER_X, WORLD_COORD_STD_DEV)
        ),
    )
    world_y = max(
        WORLD_COORD_RANGE_Y[0],
        min(
            WORLD_COORD_RANGE_Y[1], rng.gauss(WORLD_COORD_CENTER_Y, WORLD_COORD_STD_DEV)
        ),
    )
...
def generate_universe(
    ...
    default_system_template = system_templates_data.get(
        "default_system"
    ...
    for world_id, world in universe_state.worlds.items():
        if world.x is None or world.y is None:
            world.x = max(
                WORLD_COORD_RANGE_X[0],
                min(
                    WORLD_COORD_RANGE_X[1],
                    rng.gauss(WORLD_COORD_CENTER_X, WORLD_COORD_STD_DEV),
                ),
            )
            world.y = max(
                WORLD_COORD_RANGE_Y[0],
                min(
                    WORLD_COORD_RANGE_Y[1],
                    rng.gauss(WORLD_COORD_CENTER_Y, WORLD_COORD_STD_DEV),
                ),
            )
