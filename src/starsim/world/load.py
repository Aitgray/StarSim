...
from ..core.ids import (
    ...
    RecipeId,
    FactionId,
)  # Added FactionId and CommodityId
...
from ..factions.model import (
    Faction,
    WorldFactionState,
)  # Added Faction, WorldFactionState
...
def load_universe(path: Path) -> UniverseState:
    ...
    for w_data in worlds_data:
        ...
        world_factions_state_instance = (
            None  # Initialize a single instance of WorldFactionState
        )
        if "factions" in w_data:
            raw_world_fac_data = w_data[
                "factions"
            ]  # This is the {'influence': ..., 'control': ...} dict
            ...
            for fac_id_str, inf_val in influence_data.items():
                ...
                if faction_id not in factions:
                    raise UniverseSchemaError(
                        f"World '{world_id}' references unknown faction '{fac_id_str}' in influence data."
                    )
            ...
            for fac_id_str, qty in garrison_data.items():
                ...
                if faction_id not in factions:
                    raise UniverseSchemaError(
                        f"World '{world_id}' references unknown faction '{fac_id_str}' in garrison data."
                    )
            ...
            if controller_id_str:
                ...
                if controller_id not in factions:
                    raise UniverseSchemaError(
                        f"World '{world_id}' references unknown faction '{controller_id_str}' as controller."
                    )
    ...
    for l_data in lanes_data:
        ...
        if a_id not in worlds:
            raise UniverseSchemaError(
                f"Lane '{l_data['id']}' references unknown world '{l_data['a']}'."
            )
        if b_id not in worlds:
            raise UniverseSchemaError(
                f"Lane '{l_data['id']}' references unknown world '{l_data['b']}'."
            )
