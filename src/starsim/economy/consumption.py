...
class Population:
    ...
    needs: Dict[CommodityId, float] = field(
        default_factory=dict
    )  # qty per capita per tick (e.g., food: 0.0001)
    ...
    consumer_goods_excess_burn_rate: float = (
        0.5  # 50% of excess consumer goods are "burned off"
    )
...
def consume(world: "World", tick: int):
    ...
    if total_food_needed > 0:
        actual_food_consumed = market.inventory.remove_clamped(
            CommodityId("food"), total_food_needed
        )
    ...
    if food_shortage_ratio > 0:
        ...
        if population.size > 0:
            ...
            population.size = math.floor(
                population.size * (1.0 + growth_amount / population.size)
            )
    ...
    total_cg_needed = (
        population.size * population.consumer_goods_required_per_capita_per_tick
    )
    cg_available = market.inventory.get(CommodityId("consumer_goods"))

    if total_cg_needed > 0:
        actual_cg_consumed = market.inventory.remove_clamped(
            CommodityId("consumer_goods"), total_cg_needed
        )
    ...
    if cg_shortage_ratio > 0:
        ...
        world.stability = max(
            0.0, world.stability - cg_shortage_ratio * 0.02
        )  # Mild stability reduction
        world.prosperity = max(
            0.0, world.prosperity - cg_shortage_ratio * 0.01
        )  # Mild prosperity reduction
        world.unrest = min(
            1.0, world.unrest + cg_shortage_ratio * 0.01
        )  # Mild unrest increase
    else:
        ...
        if excess_in_market_after_consumption > 0 and population.size > 0:
            ...
            stability_bonus = (
                cg_excess_per_capita * 100.0 * 0.005
            )  # Example: 0.005 stability bonus per 100 excess CG per capita
            ...
            burn_amount = (
                excess_in_market_after_consumption
    ...
    for commodity_id, need_per_capita in population.needs.items():
        if commodity_id == CommodityId("food") or commodity_id == CommodityId(
            "consumer_goods"
        ):  # Already handled
            continue
