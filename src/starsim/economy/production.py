...
@dataclass
class Industry:
    caps: Dict[RecipeId, float] = field(
        default_factory=dict
    )  # max output per tick for each recipe
    ...
    for recipe_id, cap_amount in industry.caps.items():
        ...
        for input_id, input_qty_per_unit in recipe.inputs.items():
            if input_qty_per_unit > 0:
                ...
                max_possible_by_inputs = min(
                    max_possible_by_inputs, available_input / input_qty_per_unit
                )

        # Determine actual production units, limited by capacity, industry cap, and inputs
        production_units = min(
            cap_amount, recipe.max_production_units_per_tick, max_possible_by_inputs
        )

        if production_units > 0:
            # Consume inputs
            for input_id, input_qty_per_unit in recipe.inputs.items():
                market.inventory.remove_clamped(
                    input_id, input_qty_per_unit * production_units
                )
