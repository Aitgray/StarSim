...
def apply_upkeep(world: World, state: UniverseState):
    ...
    # Consume energy
    actual_energy_consumed = market.inventory.remove_clamped(
        CommodityId("energy"), total_energy_upkeep
    )
    ...
    if energy_deficit > 0:
        ...
        deficit_ratio = (
            energy_deficit / total_energy_upkeep if total_energy_upkeep > 0 else 0.0
        )
        ...
        for recipe_id in industry.caps.keys():
            industry.caps[recipe_id] = max(
                0.0, industry.caps[recipe_id] * (1.0 - deficit_ratio * 0.1)
            )  # 10% reduction
