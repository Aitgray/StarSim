...
def invest_civilian(world: World, state: UniverseState):
    ...
    if world.market.inventory.get(CommodityId("minerals")) >= minerals_needed:
        ...
        civilian_recipes_candidates = [
            RecipeId("farm_food"),
            RecipeId("mine_minerals"),
            RecipeId("refine_consumer_goods"),
        ]

        # Filter for recipes that are actually in this world's industry caps
        available_civilian_recipes = [
            r_id for r_id in civilian_recipes_candidates if r_id in world.industry.caps
        ]
...
def invest_military(world: World, state: UniverseState):
    ...
    if world.market.inventory.get(CommodityId("alloy")) >= alloys_needed:
        ...
        military_recipes_candidates = [
            RecipeId("refine_alloy"),
            RecipeId("assemble_alloys"),
        ]

        # Filter for recipes that are actually in this world's industry caps
        available_military_recipes = [
            r_id for r_id in military_recipes_candidates if r_id in world.industry.caps
        ]
