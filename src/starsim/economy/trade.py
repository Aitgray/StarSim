...
def build_candidate_trades(state: UniverseState) -> List[Shipment]:
    ...
    for shipment in state.active_shipments:
        incoming_by_world_commodity[
            (shipment.destination_world_id, shipment.commodity_id)
        ] += shipment.quantity
    ...
    for lane_id, lane in state.lanes.items():
        ...
        for commodity in state.commodity_registry.all_commodities():
            ...
            shipping_cost_per_unit = base_price * (
                lane.distance * 0.01 + lane.hazard * 0.05
            )
            ...
            if (
                price_a < base_price
                and price_b > base_price
                and profit_a_to_b > TRADE_THRESHOLD
            ):
                # How much to trade? For now, a fixed small amount or available surplus
                trade_qty = min(
                    market_a.inventory.get(commodity.id), 10.0
                )  # Trade up to 10 units
                if trade_qty > 0:
                    candidate_shipments.append(
                        Shipment(
                            ...
                            eta_tick=state.tick
                            + round(lane.distance),  # Simple ETA based on distance
                            lane_id=lane.id,
                        )
                    )
            ...
            if (
                price_b < base_price
                and price_a > base_price
                and profit_b_to_a > TRADE_THRESHOLD
            ):
                dest_target = market_a.targets.get(commodity.id, 10.0)
                dest_qty = market_a.inventory.get(commodity.id)
                incoming_qty = incoming_by_world_commodity.get(
                    (world_a.id, commodity.id), 0.0
                )
                dest_deficit = max(0.0, dest_target - (dest_qty + incoming_qty))
                trade_qty = min(
                    market_b.inventory.get(commodity.id), 10.0, dest_deficit
                )
                if trade_qty > 0:
                    candidate_shipments.append(
                        Shipment(
    ...
    if allow_new_trades:
        ...
        for shipment in new_shipments:
            if shipment.lane_id:
                ...
                remaining_capacity = state.lane_capacity_tracker.get_remaining_capacity(
                    shipment.lane_id, lane.capacity
                )
                ...
                if shipment.quantity <= remaining_capacity:
                    ...
                    if (
                        actual_deducted > 0
                    ):  # Only create shipment if goods were actually available
                        state.active_shipments.append(shipment)
                        state.lane_capacity_tracker.add_used_capacity(
                            shipment.lane_id, actual_deducted
                        )
                else:
