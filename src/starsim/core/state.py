...
from ..economy.commodities import (
    CommodityRegistry,
    commodity_registry,
)  # Import commodity_registry
from ..economy.recipes import (
    RecipeRegistry,
    recipe_registry,
)  # Import RecipeRegistry and recipe_registry
...
from ..events.registry import (
    EventRegistry,
    event_registry,
)  # Import EventRegistry and event_registry
...
class UniverseState:
    ...
    commodity_registry: CommodityRegistry = field(
        default_factory=lambda: commodity_registry
    )
    ...
    factions: Dict[FactionId, Faction] = field(
        default_factory=dict
    )  # Add faction registry
    active_shipments: List[Shipment] = field(
        default_factory=list
    )  # Add active shipments list
    lane_capacity_tracker: LaneCapacity = field(
        default_factory=LaneCapacity
    )  # Add lane capacity tracker

    # Derived adjacency list, will be rebuilt
    _adj: Dict[WorldId, List[LaneId]] = field(
