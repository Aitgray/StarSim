...
def step(state: UniverseState) -> TickReport:
        ...
        if world.population and world.market:
            ...
            if world.stability != initial_stability or world.unrest != initial_unrest:
                log.add_entry(
                    ...
                    details={
                        "old_stability": initial_stability,
                        "new_stability": world.stability,
                        "old_unrest": initial_unrest,
                        "new_unrest": world.unrest,
                    },
                )
        else:
            log.add_entry(
                ...
                state.tick,
                world_id=world.id,
                reason="No population or market, no consumption.",
            )
    ...
    for world in state.worlds.values():
        if world.industry and world.market:
            initial_inventory = {
                c_id: qty for c_id, qty in world.market.inventory.to_dict().items()
            }
            produce(world, state)
            final_inventory = world.market.inventory.to_dict()

            produced_items = {
                c_id: final_inventory.get(c_id, 0) - initial_inventory.get(c_id, 0)
            ...
            log.add_entry(
                ...
                state.tick,
                world_id=world.id,
                reason="No industry or market, no production.",
            )
    ...
    for world in state.worlds.values():
        if world.industry and world.market:
            ...
            log.add_entry(
                ...
                state.tick,
                world_id=world.id,
                reason="Investment decisions made.",
            )
        else:
            log.add_entry(
                "economy.investment",
                state.tick,
                world_id=world.id,
                reason="No industry or market, no investment.",
            )
    ...
    for world in state.worlds.values():
        if world.market and world.population and world.industry:
            ...
            log.add_entry(
                ...
                state.tick,
                world_id=world.id,
                reason="No market, population, or industry, no upkeep.",
            )
    ...
    if arrived_shipments:
        log.add_entry(
            ...
            details={
                "arrivals": [{s.commodity_id: s.quantity} for s in arrived_shipments]
            },
        )
    else:
        log.add_entry(
            "economy.trade", state.tick, reason="No active trade or arrivals."
        )
    ...
    for world in state.worlds.values():
        if world.market:
            ...
            for c_id, new_price in world.market.prices.items():
                old_price = initial_prices.get(
                    c_id, state.commodity_registry.get(c_id).base_price
                )
            ...
            log.add_entry(
                ...
                state.tick,
                world_id=world.id,
                reason="No market in world, no price update.",
            )
    ...
    if events_this_tick:
        ...
        log.add_entry(
            "events.roll", state.tick, reason="No events triggered this tick."
        )
