...
def test_generate_world_card_report_basic():
    ...
    dummy_world.population = Population(
        size=5_000_000, growth_rate=0.01
    )  # Added growth_rate for pop trend
    ...
    market = Market(
        inventory=inventory,
        prices={CommodityId("food"): 10.0, CommodityId("minerals"): 5.0},
    )
    ...
    assert (
        f"--- World Report: Alpha Centauri Prime ({WorldId('test-world-23')}) ---"
        in report
    )
