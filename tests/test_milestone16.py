...
def world_with_alloy_refinery() -> World:
    world = World(
        id=WorldId("test_alloy_refinery_world"), name="Test Alloy Refinery World"
    )
    ...
    industry = Industry(
        caps={
            RecipeId(
                "refine_alloy"
            ): 10.0,  # Produces 10 alloy units per production unit (needs 50 minerals, 20 energy)
        }
    )
...
def world_with_cg_refinery() -> World:
    ...
    industry = Industry(
        caps={
            RecipeId(
                "refine_consumer_goods"
            ): 10.0  # Produces 10 consumer goods units per production unit (needs 30 minerals, 10 energy)
        }
    )
    world.industry = industry
...
def test_refine_alloy_produces_alloy_consumes_minerals_energy(
    universe_state_for_alloy_refinery,
):
    world = universe_state_for_alloy_refinery.worlds[
        WorldId("test_alloy_refinery_world")
    ]
    ...
    assert world.market.inventory.get(CommodityId("minerals")) == pytest.approx(
        initial_minerals - 50.0
    )
    assert world.market.inventory.get(CommodityId("energy")) == pytest.approx(
        initial_energy - 20.0
    )
    assert world.market.inventory.get(CommodityId("alloy")) == pytest.approx(
        initial_alloy + 10.0
    )


def test_refine_consumer_goods_produces_cg_consumes_minerals_energy(
    universe_state_for_cg_refinery,
):
    ...
    assert world.market.inventory.get(CommodityId("minerals")) == pytest.approx(
        initial_minerals - 30.0
    )
    assert world.market.inventory.get(CommodityId("energy")) == pytest.approx(
        initial_energy - 10.0
    )
    assert world.market.inventory.get(CommodityId("consumer_goods")) == pytest.approx(
        initial_consumer_goods + 10.0
    )
