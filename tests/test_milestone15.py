...
def world_with_pop_and_market_cg() -> World:
    ...
    market.inventory.add(
        CommodityId("food"), 1000.0
    )  # Add sufficient food to avoid food shortages
    world.market = market
...
def test_cg_shortage_reduces_stability_prosperity_increases_unrest(
    universe_state_for_cg,
):
...
def test_cg_surplus_increases_stability_burns_excess(universe_state_for_cg):
    ...
    (
        initial_cg_in_market - total_cg_needed
    ) * world.population.consumer_goods_excess_burn_rate

    actual_cg_consumed = total_cg_needed
    excess_after_consumption = initial_cg_in_market - actual_cg_consumed
    expected_final_inventory = excess_after_consumption - (
        excess_after_consumption * world.population.consumer_goods_excess_burn_rate
    )
