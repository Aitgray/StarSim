...
def two_world_trade_state() -> UniverseState:
    ...
    lane_id = LaneId("l1-2")
    lane = Lane(
        id=lane_id, a=world1_id, b=world2_id, distance=1.0, hazard=0.0, capacity=100.0
    )
...
def test_shipments_arrive_after_travel_time(two_world_trade_state):
    ...
    initial_w2_food = state.worlds[WorldId("w2")].market.inventory.get(
        CommodityId("food")
    )
    ...
    assert (
        state.worlds[WorldId("w2")].market.inventory.get(CommodityId("food"))
        == initial_w2_food
    )
    ...
    assert state.worlds[WorldId("w2")].market.inventory.get(
        CommodityId("food")
    ) == pytest.approx(initial_w2_food + 5.0)
...
def test_capacity_limits_prevent_infinite_shipping(two_world_trade_state):
    ...
    initial_w1_food = state.worlds[WorldId("w1")].market.inventory.get(
        CommodityId("food")
    )
    ...
    assert state.worlds[WorldId("w1")].market.inventory.get(
        CommodityId("food")
    ) == pytest.approx(initial_w1_food - 5.0)
