...
def basic_economy_world() -> World:
    ...
    world.population = Population(
        size=1_000_000, growth_rate=0.01, food_required_per_capita_per_tick=0.0001
    )
...
def sim_universe(basic_economy_world: World) -> UniverseState:
    ...
    from src.starsim.economy.recipes import (
        recipe_registry,
    )  # Re-import to ensure it's loaded if not already
...
def run_simulation_ticks(universe: UniverseState, num_ticks: int):
    ...
    for _ in range(num_ticks):
        ...
        universe.tick += (
            1  # Increment tick after all worlds have processed for this tick
        )
    ...
    for world in sim_universe.worlds.values():
        if world.market and world.market.inventory:
            for commodity_id, quantity in world.market.inventory.to_dict().items():
                assert quantity >= 0.0, (
                    f"Inventory for {commodity_id} went negative ({quantity}) in {world.name}"
                )
...
def test_population_growth_with_and_without_food():  # No fixture here, create everything inside
    ...
    growth_world = World(
        id=world_id_growth, name="Growth World", stability=0.7, prosperity=0.7
    )
    growth_world.population = Population(
        size=1_000_000, growth_rate=0.01, food_required_per_capita_per_tick=0.0001
    )
    growth_world.market = Market(
        inventory=Inventory(),
        prices={
            CommodityId("food"): 10.0,
            CommodityId("minerals"): 5.0,
    ...
    growth_world.planets.append(
        Planet(type="earth", habitability=0.7, resource_potentials={}, tags=set())
    )
    ...
    growth_world.market.inventory.add(
        CommodityId("food"), 100_000.0
    )  # Ensure ample food
    num_ticks_growth = 5
    run_simulation_ticks(universe_growth, num_ticks_growth)
    assert growth_world.population.size > initial_pop_growth, (
        "Population did not grow with sufficient food."
    )
    ...
    decline_world = World(
        id=world_id_decline, name="Decline World", stability=0.7, prosperity=0.7
    )
    decline_world.population = Population(
        size=1_000_000, growth_rate=0.01, food_required_per_capita_per_tick=0.0001
    )
    decline_world.market = Market(
        inventory=Inventory(),
        prices={
            CommodityId("food"): 10.0,
            CommodityId("minerals"): 5.0,
    ...
    decline_world.planets.append(
        Planet(type="mars", habitability=0.3, resource_potentials={}, tags=set())
    )
    ...
    decline_world.market.inventory.remove_clamped(
        CommodityId("food"),
        decline_world.market.inventory.get(CommodityId("food")) + 1000.0,
    )  # Ensure no food
    num_ticks_decline = 5
    run_simulation_ticks(universe_decline, num_ticks_decline)
    assert decline_world.population.size < initial_pop_decline, (
        "Population did not decline during food shortage."
    )
    # Removed hardcoded population value
