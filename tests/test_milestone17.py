...
def test_invest_civilian_consumes_minerals_increases_cap(universe_state_for_investment):
    ...
    assert world.market.inventory.get(CommodityId("minerals")) == pytest.approx(
        initial_minerals - 10.0
    )  # Consumes 10 minerals
    assert (
        world.industry.caps.get(RecipeId("mine_minerals"), 0.0)
        > initial_mine_minerals_cap
    )  # Cap increases
...
def test_invest_military_consumes_alloys_increases_cap(universe_state_for_investment):
    ...
    initial_assemble_alloys_cap = world.industry.caps.get(
        RecipeId("assemble_alloys"), 0.0
    )
    ...
    assert (
        world.industry.caps.get(RecipeId("assemble_alloys"), 0.0)
        > initial_assemble_alloys_cap
    )  # Cap increases
