...
def test_generate_gazette_report_with_shortages_and_booms():
    ...
    assert (
        "No significant shortages reported." not in report
    )  # Ensure this message is not there

    # Assert Top 3 Booms
    assert "\n--- Top 3 Booms ---\n" in report
    assert "1. Consumer_goods: 550.00 units surplus" in report
    assert (
        "No significant booms reported." not in report
    )  # Ensure this message is not there
    ...
    assert (
        "Feature not yet implemented. Requires more complex simulation and historical tracking.\n"
        in report
    )
