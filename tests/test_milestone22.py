...
from src.starsim.generation.bootstrap import (
    apply_planet_potentials_to_world,
    BASE_POPULATION,
    PRODUCTION_CAP_SCALING_FACTOR,
)
...
def test_apply_planet_potentials_initializes_world_attributes():
    ...
    dummy_world = World(
        id=WorldId("test-world"), name="Test World", planets=[dummy_planet]
    )
    ...
    expected_population_size = max(
        1, floor(BASE_POPULATION * dummy_planet.habitability)
    )
    ...
    assert dummy_world.industry.caps[RecipeId("mine_minerals")] == pytest.approx(
        dummy_planet.resource_potentials[CommodityId("minerals")]
        * PRODUCTION_CAP_SCALING_FACTOR
        + dummy_planet.resource_potentials[CommodityId("energy")]
        * PRODUCTION_CAP_SCALING_FACTOR
        * 0.5
    )
    ...
    assert dummy_world.industry.caps[RecipeId("farm_food")] == pytest.approx(
        dummy_planet.resource_potentials[CommodityId("food")]
        * PRODUCTION_CAP_SCALING_FACTOR
        + dummy_planet.resource_potentials[CommodityId("energy")]
        * PRODUCTION_CAP_SCALING_FACTOR
        * 0.5
