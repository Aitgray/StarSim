...
def world_with_population_and_market() -> World:
    ...
    market.inventory.add(
        CommodityId("consumer_goods"), 1000.0
    )  # Add sufficient consumer goods
    market.prices[CommodityId("food")] = commodity_registry.get(
        CommodityId("food")
    ).base_price
    world.market = market
...
def test_consumption_removes_from_inventory(universe_state_for_consumption):
    ...
    expected_consumed = (
        world.population.size * world.population.food_required_per_capita_per_tick
    )
    assert world.market.inventory.get(CommodityId("food")) == pytest.approx(
        initial_food_in_market - expected_consumed, abs=1e-3
    )


def test_shortage_ratio_calculation_insufficient_inventory(
    universe_state_for_consumption,
):
    world = universe_state_for_consumption.worlds[WorldId("test_pop_world")]
    world.population.food_required_per_capita_per_tick = 0.1  # Needs 100 food
    world.population.consumer_goods_required_per_capita_per_tick = (
        0.0  # Remove CG influence
    )
    world.population.consumer_goods_excess_burn_rate = 0.0  # Remove CG influence
    world.market.inventory[CommodityId("food")] = (
        50.0  # Not enough for 100 population need
    )
    world.market.inventory[CommodityId("consumer_goods")] = (
        0.0  # Ensure no consumer goods surplus
    )
...
def test_shortage_ratio_calculation_sufficient_inventory(
    universe_state_for_consumption,
):
    world = universe_state_for_consumption.worlds[WorldId("test_pop_world")]
    world.population.food_required_per_capita_per_tick = 0.1  # Needs 100 food
...
def test_two_ticks_no_food_worsens_instability_deterministically():
    ...
    population1 = Population(
        size=100,
        needs={CommodityId("food"): 1.0},
        food_required_per_capita_per_tick=1.0,
        consumer_goods_required_per_capita_per_tick=0.0,
        consumer_goods_excess_burn_rate=0.0,
    )  # Needs 100 food
    ...
    assert state1.worlds[WorldId("w1")].stability == pytest.approx(
        state2.worlds[WorldId("w1")].stability
    )
    assert state1.worlds[WorldId("w1")].unrest == pytest.approx(
        state2.worlds[WorldId("w1")].unrest
    )
    assert state1.worlds[WorldId("w1")].scarcity == pytest.approx(
        state2.worlds[WorldId("w1")].scarcity
    )
