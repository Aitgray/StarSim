...
def test_production_limited_by_capacity(universe_state_for_production):
    ...
    assert market.inventory.get(CommodityId("food")) == pytest.approx(
        initial_food + 1000.0
    )
...
def test_production_limited_by_input_availability(universe_state_for_production):
    ...
    assert market.inventory.get(CommodityId("minerals")) == pytest.approx(
        initial_minerals - 50.0
    )
    assert market.inventory.get(CommodityId("alloy")) == pytest.approx(
        initial_alloys + 5.0
    )
...
def test_agri_world_produces_food_and_stabilizes_self_integration():
    ...
    population = Population(
        size=1_000, needs={CommodityId("food"): 0.1}
    )  # Needs 100 food per tick
    agri_world.population = population

    industry = Industry(
        caps={RecipeId("farm_food"): 1.5}
    )  # Produces 150 food per tick (1.5 * 100)
    ...
    assert (
        agri_world.market.inventory.get(CommodityId("food")) > 50.0
    )  # Should have produced more food
