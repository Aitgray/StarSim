...
def test_price_remains_within_bounds(universe_state_with_market):
    ...
    market.prices[food_commodity.id] = (
        food_commodity.base_price * 0.1
    )  # Below min bound
    market.prices[minerals_commodity.id] = (
        minerals_commodity.base_price * 5.0
    )  # Above max bound
    ...
    assert (
        min_minerals_price <= market.prices[minerals_commodity.id] <= max_minerals_price
    )
...
def test_world_with_missing_prices_sets_defaults_without_crash(universe_yaml_path):
    ...
    assert (
        sol_world.market.prices[alloy_commodity.id]
        >= alloy_commodity.base_price * sol_world.market.min_price_multiplier
    )
