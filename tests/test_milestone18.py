...
def test_energy_upkeep_consumes_energy(universe_state_for_upkeep):
    ...
    assert world.market.inventory.get(CommodityId("energy")) == pytest.approx(
        initial_energy - expected_consumed_energy
    )


def test_energy_deficit_reduces_stability_prosperity_throttles_caps(
    universe_state_for_upkeep,
):
    world = universe_state_for_upkeep.worlds[WorldId("test_upkeep_world")]

    # Set energy to a very low amount to ensure deficit
    world.market.inventory[CommodityId("energy")] = (
        5.0  # Needed 11.5, available 5.0 -> deficit
    )
    ...
    assert (
        world.industry.caps.get(RecipeId("mine_minerals")) < initial_mine_minerals_cap
    )
